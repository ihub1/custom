...
source <(curl -sSL 'https://raw.githubusercontent.com/ihub1/custom/main/module/module')

time_reboot() {
  print_center -ama "${a92:-Server will restart In} $1 ${a93:-Seconds}"
  REBOOT_TIMEOUT="$1"
  ...
  echo -e "\033[01;31m\033[1;33m Check me out on \033[1;31m(\033[1;36mTelegram\033[1;31m): \033[1;37m@icode1\033[0m"
  reboot
}
...
if [ "$(lsb_release -rs)" = "8*|9*|10*|11*|16.04*|18.04*" ]; then
  ...
  clear
  echo ""
  print_center -ama "Great! your Server can work for this"
  print_center -ama " ⇢ Lets begin...! <"
  sleep 3
 ...
 # [+get files ⇣⇣⇣+]
  source <(curl -sSL 'https://raw.githubusercontent.com/ihub1/custom/main/module/module') &>/dev/null
  wget -O /etc/UDPCustom/module 'https://raw.githubusercontent.com/ihub1/custom/main/module/module' &>/dev/null
  chmod +x /etc/UDPCustom/module

  wget "https://raw.githubusercontent.com/ihub1/custom/main/bin/udp-custom-linux-amd64" -O /root/udp/udp-custom &>/dev/null
  # wget "https://raw.githubusercontent.com/ihub1/custom/main/bin/udp-request-linux-amd64" -O /usr/bin/udp-request &>/dev/null
  chmod +x /root/udp/udp-custom
  # chmod +x /usr/bin/udp-request

  wget -O /etc/limiter.sh 'https://raw.githubusercontent.com/ihub1/custom/main/module/limiter.sh'
  cp /etc/limiter.sh /etc/UDPCustom
  chmod +x /etc/limiter.sh
  chmod +x /etc/UDPCustom
  
  # [+udpgw+]
  wget -O /etc/udpgw 'https://raw.githubusercontent.com/ihub1/custom/main/module/udpgw'
  mv /etc/udpgw /bin
  chmod +x /bin/udpgw

  # [+service+]
  wget -O /etc/udpgw.service 'https://raw.githubusercontent.com/ihub1/custom/main/config/udpgw.service'
  wget -O /etc/udp-custom.service 'https://raw.githubusercontent.com/ihub1/custom/main/config/udp-custom.service'
  ...
  # [+config+]
  wget "https://raw.githubusercontent.com/ihub1/custom/main/config/config.json" -O /root/udp/config.json &>/dev/null
  chmod +x /root/udp/config.json

  # [+menu+]
  wget -O /usr/bin/udp 'https://raw.githubusercontent.com/ihub1/custom/main/module/udp' 
  chmod +x /usr/bin/udp
  ...
  echo ""
  echo ""
  print_center -ama "${a103:-setting everything up for you...}"
  sleep 3
  title "${a102:-We are done!}"
